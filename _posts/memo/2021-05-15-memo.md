...
- exec() : 쿼리를 프로미스(비동기)처리를 해주기 위해서 사용함 몽고4버전부턴 생략가능 하다고는 하는데, 그래도 써주는거를 추천함<br/>
또, async/await로 대체 가능하다.<br /><br />

- find()와 findById() : find는 조건에 해당하는 모든것을 처리해주고, findById는 조건에 해당하는 id를 찾아준다고 한다. <br/>
